...
bool CompareUID(uchar *uid1, uchar *uid2, uchar length) {
...
void SaveUID(uchar *src, uchar length) {
    uidLength = length;
    for (int i = 0; i < length; i++) {
    	lastUID[i] = src[i];
    }
...
void RFID_Process(void) {
    ...
    if (status == MI_OK) {		// Card 있음
    	if (!cardPresent) {		// Card Tag
    		status = MFRC522_Anticoll(str);
    		if (status == MI_OK) {
    			size = MFRC522_SelectTag(str);
    			if (size != 0) {
    				SaveUID(str, size);
    				cardPresent = true;
