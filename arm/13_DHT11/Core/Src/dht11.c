...
void dht11Init(DHT11 *dht, GPIO_TypeDef *port, uint16_t pin) {
	dht->port = port;			// Signal Port
	dht->pin = pin;			// Signal Pin Number
}
...
uint8_t dht11Read(DHT11 *dht) {
	...
	uint8_t pulse[40] = {0};						// 40-bit Data 저장 변수
	...
	for (uint8_t i = 0; i < 8; i++) {
		humValue1 = (humValue1 << 1) + pulse[i];			// RH 상위 8-bit
	}
	for (uint8_t i = 8; i < 16; i++) {
		humValue2 = (humValue2 << 1) + pulse[i];			// RH 하위 8-bit
	}
	for (uint8_t i = 16; i < 24; i++) {
		tempValue1 = (tempValue1 << 1) + pulse[i];		// T 상위 8-bit
	}
	for (uint8_t i = 24; i < 32; i++) {
		tempValue2 = (tempValue2 << 1) + pulse[i];		// T 하위 8-bit
	}
	for (uint8_t i = 32; i < 40; i++) {
		parityValue = (parityValue << 1) + pulse[i];		// CS 8-bit
	}

	// 구조체 온습도 Data 입력
	dht->temperature = tempValue1;		// 온도 Data 저장
	dht->humidity = humValue1;			// 습도 Data 저장
